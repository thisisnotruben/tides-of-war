...
namespace Game.Ui
{
	public class InGameMenu : Menu
	{
		...
		public void _OnSpellBookPressed()
		{
			...
			spellMenu.GetNode<Label>("s/v/m/v/label").Text = $"Health: {player.hp} / {player.stats.hpMax}";
			spellMenu.GetNode<Label>("s/v/m/v/label2").Text = $"Mana: {player.mana} / {player.stats.manaMax}";
			menu.Hide();
			GetNode<Control>("c/game_menu").Show();
			spellMenu.Show();
		}
		...
		public void _OnCastPressed()
		{
			...
			//             showPopup = true;
			//         }
			//         else if (player.pos.DistanceTo(player.target.pos) > selectedSpell.spellRange &&
